"""Owner-authenticated Premier League paper simulation API."""
import hmac,json,os
...
def owner(authorization:str|None=Header(default=None)):
 expected=os.environ.get('MOBILE_OWNER_TOKEN','');actual=authorization.removeprefix('Bearer ') if authorization else ''
 if len(expected)<32:raise HTTPException(503,'Server access token has not been configured')
 if not hmac.compare_digest(expected,actual):raise HTTPException(401,'Invalid access token')
def serialize(b):
...
def create_app(setup=True):
 ...
 @app.get('/api/v1/summary',dependencies=[Depends(owner)])
 def summary():
  with connect() as c:
 ...
 @app.get('/api/v1/predictions',dependencies=[Depends(owner)])
 def predictions():
  with connect() as c:
   answer=[]
   for m in matches(c):
    p=forecast(c,m)
    if p:answer.append({'id':m['id'],'home':m['home_name'],'away':m['away_name'],'competition':'Premier League','kickoff':m['kickoff'],'model':'Dixon-Coles','selections':discrepancies(c,m,p),'stats':{'home':{'xg':p['home_goals'],'form':recent_form(c,m['home'])},'away':{'xg':p['away_goals'],'form':recent_form(c,m['away'])}}})
   return answer
 @app.get('/api/v1/bets',dependencies=[Depends(owner)])
 def bets():
  with connect() as c:
 ...
 @app.get('/api/v1/engine',dependencies=[Depends(owner)])
 def engine():
  with connect() as c:return engine_status(c)
 @app.post('/api/v1/refresh-all',dependencies=[Depends(owner)])
