...
class MobileOdds:
 ...
 def refresh(self,from_time=None,to_time=None):
  ...
  observed=now()
  with connect() as c:
   count=parse_events(c,events,observed);set_setting(c,'odds_api_quota',{'remaining':r.headers.get('x-requests-remaining'),'checked_at':observed});set_setting(c,'last_odds_refresh',observed)
