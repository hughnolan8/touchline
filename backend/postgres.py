...
CREATE TABLE IF NOT EXISTS fixture_odds_snapshots(match_id TEXT PRIMARY KEY REFERENCES matches(id),captured_at TEXT NOT NULL,payload TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS model_evaluations(id BIGSERIAL PRIMARY KEY,model_version TEXT NOT NULL,created_at TEXT NOT NULL,started_at TEXT,ended_at TEXT,fixtures INTEGER NOT NULL,metrics TEXT NOT NULL,benchmark TEXT NOT NULL,eligible INTEGER NOT NULL DEFAULT 0,UNIQUE(model_version,started_at,ended_at));
...
CREATE TABLE IF NOT EXISTS bet_notifications(bet_id BIGINT PRIMARY KEY REFERENCES bets(id),status TEXT NOT NULL DEFAULT 'pending',attempts INTEGER NOT NULL DEFAULT 0,sent_at TEXT,last_error TEXT);
CREATE TABLE IF NOT EXISTS fixture_notifications(match_id TEXT PRIMARY KEY REFERENCES matches(id),missing TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'pending',attempts INTEGER NOT NULL DEFAULT 0,sent_at TEXT,last_error TEXT);
CREATE TABLE IF NOT EXISTS bet_closing_lines(bet_id BIGINT PRIMARY KEY REFERENCES bets(id),quote_id BIGINT REFERENCES quotes(id),captured_at TEXT NOT NULL,odds DOUBLE PRECISION NOT NULL,bookmaker TEXT NOT NULL,quoted_at TEXT NOT NULL,clv DOUBLE PRECISION NOT NULL);
CREATE TABLE IF NOT EXISTS decisions(id BIGSERIAL PRIMARY KEY,match_id TEXT REFERENCES matches(id),prediction_id BIGINT REFERENCES predictions(id),created_at TEXT NOT NULL,model_version TEXT NOT NULL,decision TEXT NOT NULL,reason TEXT NOT NULL,snapshot TEXT NOT NULL,UNIQUE(match_id,model_version,decision));
