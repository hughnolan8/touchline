...
with connect() as connection:
    if args.model != 'baseline-v1':
        evaluation = connection.execute('SELECT * FROM model_evaluations WHERE model_version=? ORDER BY id DESC LIMIT 1', (args.model,)).fetchone()
    ...
    strategy = setting(connection, 'strategy')
    set_setting(connection, 'strategy', {**strategy, 'active_model': args.model, 'version': strategy.get('version', 1) + 1})
